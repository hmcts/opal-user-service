...
class UserPermissionsControllerGetIntegrationTest extends AbstractIntegrationTest {
    ...
    private static final String URL_BASE_V2 = "/v2/users";
    ...
    @ParameterizedTest
    @ValueSource(booleans = {false, true})
    @DisplayName("Should return 200 and V1 payload for generated user state route")
    void getUserStateV1_whenUserHasPermissions_returns200AndV1Payload(Boolean newLogin) throws Exception {
        long userIdWithPermissions = 500000000L;
        JwtAuthenticationToken jwtAuthenticationToken = createJwtPrincipal();
        SecurityContextHolder.getContext().setAuthentication(jwtAuthenticationToken);

        try {
            MockHttpServletRequestBuilder builder = get(URL_BASE + "/" + userIdWithPermissions + "/state")
                .principal(jwtAuthenticationToken)
                .header(X_NEW_LOGIN, newLogin);
            ResultActions actions = mockMvc.perform(builder);

            String body = actions.andReturn().getResponse().getContentAsString();
            log.info(":getUserStateV1_whenUserHasPermissions_returns200AndV1Payload: Response body:\n{}",
                     toPrettyJson(body));
            ...
            VerificationMode mode = newLogin ? times(1) : never();
            verify(eventLoggingService, mode).logEvent(any(), any(), any(), any(), any(), any());
        } finally {
    ...
    @ParameterizedTest
    @ValueSource(booleans = {false, true})
    @DisplayName("Should return 200 and V2 payload for generated user state route")
    void getUserStateV2_whenUserHasPermissions_returns200AndV2Payload(Boolean newLogin) throws Exception {
        long userIdWithPermissions = 500000000L;
        JwtAuthenticationToken jwtAuthenticationToken = createJwtPrincipal();
        SecurityContextHolder.getContext().setAuthentication(jwtAuthenticationToken);

        try {
            MockHttpServletRequestBuilder builder = get(URL_BASE_V2 + "/" + userIdWithPermissions + "/state")
                .principal(jwtAuthenticationToken)
                .header(X_NEW_LOGIN, newLogin);
            ResultActions actions = mockMvc.perform(builder);

            String body = actions.andReturn().getResponse().getContentAsString();
            log.info(":getUserStateV2_whenUserHasPermissions_returns200AndV2Payload: Response body:\n{}",
                     toPrettyJson(body));

            actions.andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(header().string("ETag", "\"0\""))
                .andExpect(jsonPath("$['cache_name']").value("opal-test@HMCTS.NET"))
                .andExpect(jsonPath("$['user_id']").value(500000000))
                .andExpect(jsonPath("$['username']").value("opal-test@HMCTS.NET"))
                .andExpect(jsonPath("$['domains']['fines']['business_unit_users']", hasSize(3)))
                .andExpect(jsonPath("$['domains']['fines']['business_unit_users'][*]['business_unit_id']",
                                    containsInAnyOrder(70, 68, 61)))
                .andExpect(jsonPath(
                    "$['domains']['fines']['business_unit_users'][?(@['business_unit_id']==70)]['roles'][*]",
                    containsInAnyOrder("Account Enquiry - Account Notes", "Account Enquiry")))
                .andExpect(jsonPath(
                    "$['domains']['fines']['business_unit_users'][?(@['business_unit_id']==68)]['roles'][0]")
                               .value("Account Enquiry - Account Notes"))
                .andExpect(jsonPath(
                    "$['domains']['fines']['business_unit_users'][?(@['business_unit_id']==61)]['roles'][0]")
                               .value("Collection Order"))
                .andExpect(jsonPath("$['domains']['fines']['business_unit_users'][0]['business_unit_name']")
                               .doesNotExist())
                .andExpect(jsonPath("$['version']").doesNotExist())
                .andExpect(jsonPath("$['business_unit_users']").doesNotExist());
